...
from listentui.downloader.baseInterface import Scores, SearchProvider, SongMetadata
...
class YoutubeMusic(SearchProvider):
    ...
    def clean(self, song: Song, title: str) -> str:
        artists = self.get_artist_variation(song)
        characters = self.get_character_variation(song)
    ...
    def find_best(self, song: Song) -> tuple[SongMetadata | None, list[SongMetadata]]:  # noqa: PLR0915
        results = self.find(song)

        scored_result: list[SongMetadata] = []
        for result in results:
            ...
            clean_title = self.clean(song, result.title)
            ...
            if song.title:
                original_title_score = max(self.ratio(song.title, title) for title in title_variation)
            if song.title_romaji:
                romanised_title_score = max(self.ratio(song.title_romaji, title) for title in title_variation)
            ...
            matched: list[float] = []
            for artist in self.get_artist_variation(song):
            ...
            artist_score = sum(matched) / max(len(matched), 1) * 100 - (
                (max(len(self.get_artist_variation(song)) - len(matched), 0)) * self.artist_punish
            )

            # compares album if an album is present
            album_score = 0
            if result.album and song.album:
                album_score = max(self.ratio(album, result.album) for album in self.get_album_variation(song)) * 100
            elif result.album and any(
                result.album.lower() in album.lower() for album in self.get_album_variation(song)
            ):
                album_score = 100
            ...
            duration_score = 0
            if result.duration and song.duration:
                one_percentile = song.duration * 0.01
                five_percentile = song.duration * 0.05
                difference = abs(result.duration - song.duration)
        ...
        passed_result: list[SongMetadata] = []
        ...
        if passed_result:
            ...
            similar = [*scored_result[1:5]]
            similar.sort(key=lambda result: result.scores.total)
            return (best, similar)

        return (None, scored_result[:3])

    def find(self, song: Song) -> list[Result]:
        song_title = self.get_title_variation(song)[0]
        song_artist = self.get_artist_variation(song)
